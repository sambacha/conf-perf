...
AGENT_LOG_ROTATE_CONFIG_FREEBSD = """
# Version: $VERSION$
/var/log/awslogs.log {
    $SU_FOR_UBUNTU$
    missingok
...
NANNY_CRON_CMD_FREEBSD = """
# Version: {VERSION}
* * * * * {AGENT_NANNY_PATH} > /dev/null 2>&1
"""
...
LOGROTATE_CRON_CMD_FREEBSD = """
# Version: {VERSION}
30 * * * * logrotate -s /var/log/logstatus /usr/local/etc/logrotate.d/awslogs
"""
...
DAEMON_SCRIPT_FREEBSD = """#!/bin/sh
# PROVIDE: awslogs
# REQUIRE: NETWORKING
# REQUIRE: LOGIN
# KEYWORD: shutdown
# Define awslogs_enable="YES" in /etc/rc.conf to enable the deamon for AWSLogs agent
...
name="awslogs"
rcvar=awslogs_enable

command="/usr/local/etc/rc.d/awslogs"
pidfile=$PIDFILE$
start_cmd="awslogs_start"
stop_cmd="awslogs_stop"
restart_cmd="awslogs_restart"
status_cmd="awslogs_status"

LOCKFILE=$LOCKFILE$
MUTEXFILE=$MUTEXFILE$

START_CMD=$AGENT_LAUNCHER$
...
awslogs_stop()
{
    (
        ...
        ppids=`ps axo pgid,command | grep "[a]wslogs-agent-l" | awk '{print $1}'`
        for pid in $ppids; do
            pkill -15 -g $pid &> /dev/null
        done

        sleep 3
        procList=`ps axo pgid,command | grep "[a]wslogs-agent-l"`
        procsAlive=$?
        ppids=`${procList} | awk '{print $1}'`
        if [ $procsAlive = 0 ]; then
            for pid in $ppids; do
                pkill -9 -g $pid &> /dev/null
            done
        fi
...
awslogs_check_status()
{
    check=`pgrep -F $pidfile > /dev/null 2>&1`
...
#
# To check running status: /usr/local/etc/rc.d/awslogs status
# To stop the agent: /usr/local/etc/rc.d/awslogs stop
# To start the agent: /usr/local/etc/rc.d/awslogs start
#
# --- AGENT LOG OUTPUT ---
...
#
# Format specifier for timestamp parsing. Here are some sample formats:
# Use '%e %d %H:%M:%S' for freebsd Logs (Dec  1 08:38:42)
# Use '%b %d %H:%M:%S' for syslog (Apr 24 08:38:42)
# Use '%d/%b/%Y:%H:%M:%S' for apache log (10/Oct/2000:13:55:36)
# Use '%Y-%m-%d %H:%M:%S' for rails log (2008-09-08 11:52:54)
datetime_format = %b %d %H:%M:%S # Specification details in the table below.
...
class CloudWatchLogsAgentSetup:
    FreeBSD, Rhel, Ubuntu, AmazonLinux, CentOS, Raspbian, Debian = range(7)
    ...
    def write_launcher_file(self):
        ...
        nicepath = subprocess.Popen(
            ['which', 'nice'], stdout=subprocess.PIPE, universal_newlines=True).communicate()[0].rstrip()
    ...
    def write_daemon_script(self):
        if self.os_flavor == self.FreeBSD:
            file_path = "/usr/local/etc/rc.d/awslogs"
            contents = DAEMON_SCRIPT_FREEBSD
        else:
    ...
    def get_distro_info(self):
        ...
        os_type = subprocess.Popen(
            ['uname', '-s'], stdout=subprocess.PIPE, universal_newlines=True).communicate()[0].rstrip()
        if os_type.startswith("FreeBSD"):
            return self.FreeBSD
        else:
            fail("Failed to determine FreeBSD distribution. Exiting.",
        ...
        if os.path.isfile(redhat_version_file_path):
            ...
            fail("Failed to determine linux distribution. Exiting.",
                 PLATFORM_NOT_SUPPORTED)
    ...
    def install(self, package):
        ...
        subprocess.call([installer, 'install', '-y', package],
    ...
    def install_awslogs_cli(self):
        ...
        if not os.path.exists(VIRTUALENV_ACTIVATE_CMD):
            ...
            venv_in_path = (subprocess.call(
                ["which", "virtualenv"], stderr=self.log_file, stdout=self.log_file) == 0)
            if venv_in_path:
                venv_cmd = ["virtualenv"]
            else:
        ...
        if self.is_standalone:
            ...
            self.do_pip_install('awscli-cwlogs==1.4.4',
                                True, plugin_path, True)
        else:
            ...
            if self.plugin_url:
                if need_wheel_29:
                    self.do_pip_install(
                        'wheel==0.29.0', True, None, True, self.plugin_url)
                self.do_pip_install('awscli-cwlogs==1.4.4',
                                    True, None, True, self.plugin_url)
            else:
                if need_wheel_29:
        ...
        subprocess.call([AWSCLI_CMD, 'configure', 'set',
                        'plugins.cwlogs', 'cwlogs'], env=DEFAULT_ENV)

        # Setup the default region for the CLI
        subprocess.call([AWSCLI_CMD, 'configure', 'set',
                        'default.region', self.region], env=DEFAULT_ENV)
        ...
        if not self.ca_bundle == "":
            subprocess.call([AWSCLI_CMD, 'configure', 'set',
                            'default.ca_bundle', self.ca_bundle], env=DEFAULT_ENV)
    ...
    def write_agent_config_file(self):
        if self.config_file.startswith("http"):
            ...
            subprocess.call(
                [AWSCLI_CMD, 's3', 'cp', self.config_file, AGENT_CONFIG_FILE], env=DEFAULT_ENV)
        elif not os.path.isfile(self.config_file):
            fail("Config file %s doesn't exist." %
                 self.config_file, CONFIG_FILE_NOT_FOUND)
        else:
    ...
    def setup_initd(self):
        ...
        if self.os_flavor == self.Debian or self.os_flavor == self.Ubuntu:
            subprocess.call(['/usr/sbin/update-rc.d', 'awslogs', 'defaults'],
                            env=DEFAULT_ENV, stderr=self.log_file, stdout=self.log_file)
    ...
    def setup_agent_as_daemon(self):
        ...
        if self.os_flavor == self.AmazonLinux or self.os_flavor == self.Ubuntu or self.os_flavor == self.Rhel or self.os_flavor == self.CentOS or self.os_flavor == self.Raspbian or self.os_flavor == self.Debian:
            self.setup_initd()
        elif self.os_flavor == self.FreeBSD:
            return
        else:
    ...
    def setup_virtualenv(self):
        ...
        if virtual_env_dir == "":
            fail("Failed to find the virtual env folder in the dependency_path folder",
                 STANDALONE_INSTALLATION_FAILURE)

        virtual_env_script = os.path.join(
            dependency_dir, virtual_env_dir, "virtualenv.py")
        ...
        subprocess.call(['service', 'awslogs', 'stop'],
                        stderr=self.log_file, stdout=self.log_file)
    ...
    def validate_dependency_path(self, path):
        if not os.path.exists(path):
            fail("Make sure the dependency_path points to the correct folder. Path doesn't exist: " +
                 path, STANDALONE_INSTALLATION_FAILURE)

    def install_system_dependencies(self):
        self.install_pip()
        self._install_ubuntu_dependencies()
        if self.os_flavor == self.FreeBSD:
            self.install("logrotate")
    ...
    def install_pip(self):
        ...
        if not self.is_standalone and not executable_exists("pip"):
            if self.os_flavor == self.Rhel or self.os_flavor == self.CentOS:
                ...
                self.install("py27-sqlite3")
                self.install("py27-pip")
            else:
        ...
        if not self.is_standalone and not executable_exists("pip"):
            fail("Could not install pip. Please try again or see " +
                 AGENT_SETUP_LOG_FILE + " for more details")

    def _package_installed(self, package_name):
        devnull = open(os.devnull, "w")
        if self.os_flavor == self.Ubuntu or self.os_flavor == self.Debian:
            ret_val = subprocess.call(
                ["dpkg", "-s", package_name], stdout=self.log_file, stderr=self.log_file)
        else:
            ret_val = subprocess.call(
                ["rpm", "-q", package_name], stdout=self.log_file, stderr=self.log_file)
        devnull.close()
        if ret_val != 0:
    ...
    def setup_agent_log_file_rotation(self):
        if self.os_flavor == self.FreeBSD:
            config = AGENT_LOG_ROTATE_CONFIG_FREEBSD
        else:
        ...
        if self.os_flavor == self.FreeBSD:
            file_path = "/usr/local/etc/logrotate.d/awslogs"
        else:
            file_path = "/etc/logrotate.d/awslogs"

        with open(file_path, "a") as log_rotation_config:
    ...
    def setup_cron_jobs(self):
        if self.os_flavor == self.FreeBSD:
            ...
            with open(cron_path, "a+") as cron_fragment:
                cron_fragment.write(NANNY_CRON_CMD_FREEBSD.format(
                    VERSION=AWSLOGS_VERSION, AGENT_NANNY_PATH=AGENT_NANNY_PATH) + "\n" +
                    LOGROTATE_CRON_CMD_FREEBSD.format(VERSION=AWSLOGS_VERSION))
            self.log_generated_file(cron_path)
        else:
    ...
    def write_nanny_script(self):
        ...
        ps_path = subprocess.Popen(
            ['which', 'ps'], stdout=subprocess.PIPE, universal_newlines=True).communicate()[0].rstrip()
        grep_path = subprocess.Popen(
            ['which', 'grep'], stdout=subprocess.PIPE, universal_newlines=True).communicate()[0].rstrip()
        cat_path = subprocess.Popen(
            ['which', 'cat'], stdout=subprocess.PIPE, universal_newlines=True).communicate()[0].rstrip()
        service_path = subprocess.Popen(
            ['which', 'service'], stdout=subprocess.PIPE, universal_newlines=True).communicate()[0].rstrip()
    ...
    def setup_daemon(self):
        ...
        # Restart the agent.
        if self.os_flavor == self.FreeBSD:
            subprocess.call(['service', 'awslogs', 'onestart'],
                            stderr=self.log_file, stdout=self.log_file)
        else:
            subprocess.call(['service', 'awslogs', 'restart'],
                            stderr=self.log_file, stdout=self.log_file)
    ...
    def write_awslogs_version_script(self):
        with open(AWSLOGS_VERSION_SCRIPT_PATH, "w") as version_script:
            version_script.write(AWSLOGS_VERSION_SCRIPT.replace(
                "$AWSLOGS_GENERATED_FILES_LOG$", AWSLOGS_GENERATED_FILES_LOG))
    ...
    def setup_artifacts(self):
        ...
        with open(AWS_PROXY_CONFIG_FILE, "w") as config:
            config.write(PROXY_CONFIG.format(http_proxy=self.http_proxy,
                         https_proxy=self.https_proxy, no_proxy=self.no_proxy, VERSION=AWSLOGS_VERSION))
        ...
        rerun_cmd = "sudo python ./awslogs-agent-setup.py --region " + \
            self.region + " --only-generate-config"
        message("\n------------------------------------------------------")
        ...
        if self.os_flavor == self.FreeBSD:
            	message("\n- Add the following line to /etc/rc.conf to enable awslogs:")
            	message('\n\n- awslogs_enable="YES"')
                message('\n\n- or run sudo sysrc awslogs_enable="YES" from the command line')
    ...
    def get_system_defaults(self):
        ...
        if self.os_flavor == self.FreeBSD:
            datetime_format = '%e %d %H:%M:%S'
        else:
            datetime_format = '%b %d %H:%M:%S'

        if self.os_flavor == self.FreeBSD or self.AmazonLinux or self.os_flavor == self.Rhel or self.os_flavor == self.CentOS:
...
class LogsConfig(object):
    ...
    def _get_config(self, file_path):
        ...
        if path is not None:
            ...
            if not os.path.isfile(path):
                self._generate_config_template(path, self)
    ...
    @staticmethod
    def _generate_config_template(path, self):
        ...
        with os.fdopen(os.open(path, os.O_WRONLY | os.O_CREAT, 0o600), 'w') as file:
            ...
            os_type = subprocess.Popen(
                ['uname', '-s'], stdout=subprocess.PIPE, universal_newlines=True).communicate()[0].rstrip()
            if os_type.startswith("FreeBSD"):
                file.writelines(DEFAULT_CONFIG_FREEBSD)
            else:
